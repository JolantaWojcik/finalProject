	...
	if(Input::exists()) { //check if the input actually exists or not, so whether the form has been submitted
		if(Token::check(Input::get('token'))) { //check if the token is correct and has been supplied by the form
			...
			if($validate->passed()) {   //check if validation passes
				...
				$user = new User();		//tworzenie nowego uzytkownika user
				// We create a remember variable
				//and we want to detect whether we have this input remembered, when it equals on or not:
				...
				$login = $user->login(Input::get('login'), Input::get('haslo'), $remember); //we process the log in by creating the login value, utilizing the user object and the login method
													    //we pass that through into our login method /remember/
